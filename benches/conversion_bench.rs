...
fn bench_audio_format_parsing(c: &mut Criterion) {
    ...
    let test_inputs = vec!["1", "2", "3", "mp3", "aac", "opus", "MP3", "AAC", "OPUS"];
...
fn bench_recursive_traversal(c: &mut Criterion) {
    ...
    for depth in depths {
        ...
        group.bench_with_input(BenchmarkId::new("recursive_find", depth), &depth, |b, _| {
            b.iter(|| {
                let files = processor.find_video_files(temp_dir.path()).unwrap();
                black_box(files);
            });
...
fn bench_memory_efficiency(c: &mut Criterion) {
    ...
    group.bench_function("large_file_list_processing", |b| {
        b.iter(|| {
            ...
            // 模拟对文件列表的处理
            let total_size: usize = files.iter().map(|path| path.to_string_lossy().len()).sum();
            ...
            for path_str in &test_paths {
                ...
                if let Some(stem) = path.file_stem() {
                    for format in AudioFormat::all_formats() {
                        let output_name =
                            format!("{}.{}", stem.to_string_lossy(), format.extension());
