...
fn create_demo_environment() -> Result<std::path::PathBuf, VideoToAudioError> {
    ...
    // 创建目录
    std::fs::create_dir_all(&temp_dir).map_err(|e| VideoToAudioError::Io(e))?;
    ...
    // 创建一些非视频文件（应该被忽略）
    let non_video_files = vec!["readme.txt", "image.jpg", "audio.mp3"];

    for file_name in non_video_files {
        let file_path = temp_dir.join(file_name);
        std::fs::write(&file_path, "其他类型的文件内容").map_err(|e| VideoToAudioError::Io(e))?;
    }

    // 创建子目录和嵌套文件
    let sub_dir = temp_dir.join("subfolder");
    std::fs::create_dir_all(&sub_dir).map_err(|e| VideoToAudioError::Io(e))?;

    let nested_file = sub_dir.join("nested_video.mp4");
    std::fs::write(&nested_file, "嵌套的视频文件").map_err(|e| VideoToAudioError::Io(e))?;
